...
Table of contents
-----------------
- [Replace heuristics with an LLM for processing input queries](#replace-heuristics-with-an-llm-for-processing-input-queries)
- [LLM routing: three design options and recommended MVP](#llm-routing-three-design-options-and-recommended-mvp)
- [Use Questions as evaluation / training data](#use-questions-as-evaluation--training-data)
- [Urgently start using images inside the project](#urgently-start-using-images-inside-the-project)
...
## Replace heuristics with an LLM for processing input queries
...
Validation / tests
- Unit test for JSON schema compliance (happy path and malformed LLM output handling).
- Smoke integration: run `QueryInterface` with `--parser llm` and an offline mocked LLM to validate a pipeline.
...
## LLM routing: three design options and recommended MVP

### Problem statement
You want to route a query to the appropriate reasoning/solver. There are three designs:

1. All-in-one LLM
2. LLM Router (classify and dispatch to 9 query handlers)
3. LLM Planner (planner that issues sub-queries and composes answers / tool-calls)
...
**LLM Router**
- Pros: Middle ground — LLM classifies intent and parameters; you still use deterministic KG solvers for execution (best of both worlds).
- Cons: Depends on LLM classification quality; requires robust schema enforcement.
...
### MVP recommendation
Implement the LLM Router first (option 2). It gives immediate quality gains while keeping deterministic, debuggable execution in the existing `GQA_Reasoning_Engine`.
...
### Files to add / change
- Add: `src/llm_router.py` (router adapter and prompt)
- Add: `scripts/router_eval.py` — run batch queries and measure top-1/type accuracy and a confusion matrix
...
### Validation / tests
- Create a small labeled dataset (see section 3). Evaluate precision/recall; iterate with prompt engineering.

### Risks
- Misclassification still possible — keep a fallback path to rule-based parser and/or human-in-loop.

## Use Questions as evaluation / training data

### Goal
Turn Questions (example queries) into structured assets for evaluation, few-shot examples, and potential fine-tuning or retrieval-augmented prompts.

### Actions
- Curate a dataset `data/questions.jsonl` with fields: `id`, `query`, `gold_type`, `gold_params`, `gold_answer_id(s)` (if available), `notes`.
- Use them for:
  1. Router training/eval (classification label = `gold_type`)
  2. LLM parser few-shot examples (inject 4–12 examples into prompts)
  3. Evaluation harness `scripts/evaluate_retrieval.py` to compute Recall@k, MRR, accuracy on sample queries
- Use LLMs to augment Questions: generate paraphrases (a few per query) to increase robustness; verify paraphrases via a vetting script or human review.
...
### Validation / tests
- Run `scripts/router_eval.py --limit 200` and produce a confusion matrix and list of failed cases for prompt improvements.

## Urgently start using images inside the project

### Problem
Current project is primarily graph/text-based; integrate images urgently with a minimal, practical MVP and iterate.
...
- **MVP A (fastest, smallest changes):** Use CLIP (OpenAI CLIP / OpenCLIP) to compute global image embeddings and index them. For each returned graph result, return the image embedding distance as an additional signal to re-rank. This quickly stitches images into retrieval without changing the KG.
...
Implement MVP A and B in two steps: first implement CLIP embeddings and index (quick), then add BLIP captioning to enrich KG nodes. This gives immediate image-signal for ranking and a path to grounded text.
...
- Finalize tests and a small CI workflow to run smoke tests.
...
-----------------------------------
- Acceptance: router classification accuracy > 85% on an initially labeled set; image re-ranking improves top-5 recall in human spot-checks; integration tests pass.
...
The next immediate action I will take if you want me to proceed
---------------------------------------------------------
